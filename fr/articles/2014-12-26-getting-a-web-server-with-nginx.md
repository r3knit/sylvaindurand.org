...
Dans la ligne de [l'article précédent](/installer-archlinux-sur-raspberry-pi/), nous allons voir comment installer un *Nginx* capable d'utiliser PHP sur un Raspberry Pi sur lequel est installé *Archlinux*. Néanmoins, la procédure restera très similaire sur une autre machine ou une autre configuration{{% marginalia %}}ce sont essentiellement les commandes `pacman` et `systemctl` qui devront être adaptées{{% /marginalia %}}.
...
En ouvrant l'adresse IP{{% marginalia %}}ou le nom de domaine qui pointe vers votre Raspberry Pi{{% /marginalia %}} de votre Raspberry Pi, vous devriez alors voir apparaître une page attestant que *Nginx* est bien fonctionnel.
...
Le paramètre `listen` indique le port d'écoute{{% marginalia %}}on choisit généralement le port 80 pour du HTTP classique, ou 443 pour du HTTPS{{% /marginalia %}}, `server_name` permet de déclarer le(s) chemin(s) d'accès au site, `root` les emplacements des fichiers et `index` le fichier à servir par défaut.
...
Les sites statiques, [c'est bien](/site-statique-avec-jekyll/), mais pouvoir créer des sites dynamiques est souvent bien utile. Cela permet par exemple, depuis un *Raspberry Pi*, de pouvoir héberger chez soi des applications web comme un lecteur de flux RSS ou un *cloud*. Des exemples sont présentés ci-dessous.
