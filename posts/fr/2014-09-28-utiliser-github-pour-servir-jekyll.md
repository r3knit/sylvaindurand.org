...
Nous avons vu dans des articles précédents comment [créer un site statique avec *Jekyll*]({{ site.data.translations[page.lang].base }}/site-statique-avec-jekyll/), puis comment [le servir avec *Cloudfront*]({{ site.data.translations[page.lang].base }}/servir-son-site-avec-cloudfront/). À la place, nous allons montrer dans cet article comment :
...
Il ne vous reste désormais qu'à placer les fichiers de votre site *Jekyll* dans ce répertoire. Il peut soit s'agir d'un site que vous avez précédemment créé, soit d'un nouveau site créé avec `jekyll new`[[l'article [Site statique avec *Jekyll*]({{ site.data.translations[page.lang].base }}/site-statique-avec-jekyll/) explique comment créer un site simple sous *Jekyll*]].
