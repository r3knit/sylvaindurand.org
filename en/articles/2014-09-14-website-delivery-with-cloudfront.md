...
alt: Website delivery <br> with *Cloudfront*
aliases: /static-website-with-cloudfront/
categories: Jekyll
---
...
This article sets out to show how to host a static website{{% marginalia %}}wether built from a generator like [*Jekyll*](https://jekyllrb.com), [*Pelican*](https://blog.getpelican.com) or [*Hyde*](https://hyde.github.io), or "by hand"{{% /marginalia %}} on [*Amazon Web Services*](https://aws.amazon.com), especially [*Amazon S3*](https://aws.amazon.com/fr/) in order to store the website files, and [*CloudFront*](https://aws.amazon.com/fr/cloudfront/) in order to deliver them. The main purpose is to be able to get a website fast from anywhere, reliable, secure, highly scalable and inexpensive{{% marginalia %}}with a low traffic, the hosting will cost you about one dollar a month{{% /marginalia %}}.
...
We will use `s3cmd` in order to sync our bucket with our local files. Install the last development version{{% marginalia %}}you need to install at least the 1.5 version{{% /marginalia %}} which allows us to invalidate files on *Cloudfront*.
