---
title: Getting a web server with Nginx
alt:  Getting a web <br> server with *Nginx*
aliases: /synchronize-calendars-and-contacts-with-owncloud/
categories: Nginx
---
...
In recent years, *Nginx* has established itself as a high quality alternative: although comprehensive features, the software is characterized by great lightness, which allows to run on very modest configurations such as a Raspberry Pi.
 ...
 In line with the [previous article](/installing-archlinux-on-raspberry-pi/), we'll see how to install a *Nginx* able to use PHP, on a Raspberry Pi which is running *Archlinux*. However, the procedure will remain very similar to another machine or other configuration{{% marginalia %}}it is essentially the commands `systemctl` and `pacman` which need to be adapted{{% /marginalia %}}.
...
By opening the IP address{{% marginalia %}}or the domain name that points to your Raspberry Pi{{% /marginalia %}} of your Raspberry Pi, you should see a page showing that *Nginx* is functional.
...
The `listen` parameter specifies the listening port{{% marginalia %}}we generally chose port 80 for HTTP or 443 for HTTPS{{% /marginalia %}}, `server_name` allows to wanted URLs, `root` the file locations and `index` the file to be served by default.
