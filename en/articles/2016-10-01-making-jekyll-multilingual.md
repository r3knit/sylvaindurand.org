...
alt: Making *Jekyll* <br> multilingual
categories: Jekyll
---
...
This article aims to present a way to create a multilingual site with _Jekyll_. It has to be installed{{% marginalia %}}the article [Static website with *Jekyll*](/static-website-with-jekyll/) explains how to install and use *Jekyll* in order to get a simple website{{% /marginalia %}} on your computer and you should be able to know how to generate a simple website.
...
Our website can be translated in as many languages as wanted. Each page may be translated or not in the various languages{{% marginalia %}}regardless of the version, they may be pages which are not translated in each language{{% /marginalia %}}. All pages and posts can be placed as desired.
...
A language selector, as the one on the top right of this website, will show the actual language and the different translations available{{% marginalia %}}the selector musn't lead to the translated homepage, but to the translation of the current page{{% /marginalia %}}.
...
Then, in order to emphase the actual version, just use CSS{{% marginalia %}}you need to declare the `lang` attribute on the `html`, with `<html lang="{{ page.lang }}">` in the *layout*{{% /marginalia %}}. For instance, if you want to bold it:
...
To do so, we can provide translations into `_config.yml`{{% marginalia %}}since *Jekyll* 2.0, it is also possible to put the translations in the [_data](https://jekyllrb.com/docs/datafiles/) folder{{% /marginalia %}}. Then, in the following example, `{{ site.t[page.lang].home }}` will generate `Home`, `Accueil` or `首页` depending of the page language:
...
You only have to provide in the `<head>` part of the page, every translation available of the actual version{{% marginalia %}}you need to be careful to use the good [country codes](https://support.google.com/webmasters/answer/189077?hl=en){{% /marginalia %}}. To do so, we can use the following code, similar to those used previously:
