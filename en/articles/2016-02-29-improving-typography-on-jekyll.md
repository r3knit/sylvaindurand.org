...
alt: Improving typography <br> on *Jekyll*
categories: Jekyll
---
...
To do so, we will replace `{{ content }}`, in the `_layout/default.html` file{{% marginalia %}}or equivalent{{% /marginalia %}}, with :
...
We just have to act on the variable `t`, which corresponds to the texts, by applying different filters, just after `{% assign t = part.last %}`.
...
For quotes French `«` and `»`, we simply replace English quotation marks, as seen in the previous section, with:{{% marginalia %}}we add a normal non-breaking space before the closing quotation mark, and after entering quote with `&#160;`{{% /marginalia %}}
...
If your website is multilingual{{% marginalia %}}as explained, for example, in the article [Making *Jekyll* multilingual](/making-jekyll-multilingual/){{% /marginalia %}}, you can restrict the previous modifications to the French articles, by adding:
