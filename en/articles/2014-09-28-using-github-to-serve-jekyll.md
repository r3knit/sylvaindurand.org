---
title: Using Github to serve Jekyll
alt: Using Github <br> to serve *Jekyll*
categories: Jekyll
---
...
### Account creation
If you haven't already got one, create an account on [*GitHub*](https://github.com/) by providing an username{{% marginalia %}}beware, the username is important, because it will be used in the repository and the URL used by *GitHub Pages*{{% /marginalia %}}, an email address and a password; otherwise, use your usual login informations.
...
If you have got a *GitHub* paid plans, you can create a private repository in order to hide your codes{{% marginalia %}}in the case of a simple *Jekyll* website, this option may not be really useful{{% /marginalia %}}.
...
Let's start by creating a `.gitignore` file which will allow us to ignore the `_site` repository in which the website will be generated{{% marginalia %}}we musn't check this folder, which is only a result of the source code{{% /marginalia %}}, the `Gemfile.lock` file we will create later, and if you use on macOS the `.DS_Store` folders created by the operating system. The `.gitignore` file will looks like:
...
We can now push this file on *GitHub*{{% marginalia %}}when you `push` for the first time, you have to provide your username and your password, but they won't be asked again{{% /marginalia %}} :
...
Now, you only have to put your *Jekyll* website in this folder. It may be an existing website, or a newly created one thanks to `jekyll new`{{% marginalia %}}the article [Creating a static website with *Jekyll*](/static-website-with-jekyll/) explains how to create a simple website with *Jekyll*{{% /marginalia %}}.
...
Because the website only use simple text files, you can use *git* as you would do with any other project. Many references exists if you are not familiar with `git`{{% marginalia %}}for example, you can read [Git, a simple guide](https://rogerdudler.github.io/git-guide/) for a short introduction, or the more complete [Pro Git](https://git-scm.com/book/en/v2) book{{% /marginalia %}}.
